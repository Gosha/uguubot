...
# def db_init(db,chan):
#     db.execute("CREATE TABLE if not exists channels(channel, admins, permissions, aop, bans, disabled, ignored, badwords, flood, cmdflood, trim, primary key(channel))")
#     try: db.execute("select channel from channels where channel=lower(?)", [chan]).fetchone()[0]
#     except: db.execute("INSERT INTO channels VALUES(?,' ',' ',' ',' ',' ',' ',' ',' ',' ',' ')", [chan])
#     db.commit()


@hook.command(channeladminonly=True)
def host(inp, conn=None, db=None):
...
# @hook.command('timeoutso much drama', permissions=["op_ban", "op"], channeladminonly=True)
@hook.command('kb', permissions=["op_ban", "op"], channeladminonly=True)
@hook.command(permissions=["op_ban", "op"], channeladminonly=True)
def ban(inp, conn=None, chan=None, notice=None, db=None):
    """ban [channel] <user> [timer] -- Makes the bot ban <user> in [channel].
    ...
    conn.send(u"MODE {} {} {}".format(chan, mode, target))
    conn.send(u"KICK {} {} :{}".format(chan, nick, reason))
...
def up(inp, conn=None, chan=None, notice=None, nick=None):
    ...
    If [channel] is blank the bot will op <user> in
    the channel the command was used in."""
    mode_cmd("+o", "op", nick, chan, conn, notice)
...
def down(inp, conn=None, chan=None, notice=None, nick=None):
    ...
    If [channel] is blank the bot will deop <user> in
    the channel the command was used in."""
    mode_cmd("-o", "deop", nick, chan, conn, notice)
